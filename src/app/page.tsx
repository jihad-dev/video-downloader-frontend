...
export default function Home() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const resp = await axios.post(`${BACKEND_BASE}/download`, { url: link });
      ...
      console.error(err);
      showAlert('error', `Something went wrong: ${err.response?.data?.detail || err.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      ...
      {alert && (
        <div className={`fixed top-2 left-2 right-2 sm:top-4 sm:left-4 sm:right-4 z-50 p-3 sm:p-4 rounded-2xl shadow-2xl transform transition-all duration-300 backdrop-blur-sm ${alert.type === 'success' ? 'bg-emerald-500/90 text-white border border-emerald-400' :
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <button onClick={() => setAlert(null)} className="ml-2 text-white/80 hover:text-white transition-colors flex-shrink-0">✕</button>
          </div>
        </div>
      )}
      ...
      {/* Main Container */}
      <div className="max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl mx-auto min-h-screen flex flex-col px-2 sm:px-4 md:px-6">
        {/* Header, Input & Download Button */}
        {/* ... আপনার আগের JSX code এখানে রাখুন, শুধু handleDownload call ঠিক থাকবে ... */}
        <input
          type="url"
          placeholder="https://youtube.com/watch?v=..."
          value={link}
          onChange={(e) => setLink(e.target.value)}
          className="w-full pl-4 pr-4 py-3 border rounded-xl"
        />
        <button onClick={handleDownload} disabled={!isButtonEnabled}>
        ...
        {downloadUrl && (
          <div>
            ...
            <button onClick={resetForm}>New Video</button>
          </div>
        )}
      </div>
    </div>
